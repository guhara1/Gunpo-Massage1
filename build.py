...
from content.site import (BASE_URL, BRAND, BRAND_MARK, GOOGLE_VERIFICATION,
                          HOME, INDEXNOW_KEY, NAV, NAVER_VERIFICATION, PHONE,
                          PHONE_DISPLAY, TELEGRAM_URL)

ROOT = os.path.dirname(os.path.abspath(__file__))
MIN_INDEX_CHARS = 2000

# 검색엔진 소유확인 메타 (모든 페이지 head 에 삽입)
_VERIFY_META = ""
...
def render_breadcrumb(crumbs) -> str:
    ...
    parts.append(f'<li><a href="{HOME}">군포 홈</a></li>')
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">군포시 전지역 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      ...
      <div class="footer-inquiry">
        <a class="inquiry-btn" href="{TELEGRAM_URL}" target="_blank" rel="noopener nofollow">웹사이트 제작문의 ↗</a>
        <a class="inquiry-btn" href="{TELEGRAM_URL}" target="_blank" rel="noopener nofollow">제휴문의 ↗</a>
      </div>
    </div>
  ...
  <div class="footer-bottom">
    <div class="container footer-bottom-inner">
      ...
      <a class="footer-made" href="{TELEGRAM_URL}" target="_blank" rel="noopener nofollow">텔레그램 문의 ↗</a>
    </div>
  </div>
</footer>
...
def build() -> None:
    report = []
    sitemap_pages = []   # (loc, title, desc) — 색인 허용 페이지만
    ...
    for page in PAGES:
        path = page["path"]  # "" 또는 "sanbon-dong/" 형태
        ...
        noindex = page.get("noindex", False) or chars < MIN_INDEX_CHARS
        if not noindex:
            sitemap_pages.append((base + "/" + path, page["title"], page["desc"]))
        report.append((path or "/", chars, "noindex" if noindex else "index"))

    # sitemap.xml (lastmod 포함 — 신선도 신호)
    urls = "\n".join(
        f"  <url><loc>{loc}</loc><lastmod>{today}</lastmod>"
        f"<changefreq>weekly</changefreq></url>"
        for loc, _, _ in sitemap_pages
    )
    ...
    items = "\n".join(
        ...
        "</item>"
        for loc, title, desc in sitemap_pages
    )
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "  <channel>\n"
            f"    <title>{html.escape(BRAND)} — 군포시 출장마사지·홈타이 안내</title>\n"
            f"    <link>{base}/</link>\n"
            f'    <atom:link href="{base}/rss.xml" rel="self" type="application/rss+xml"/>\n'
            "    <description>군포시 방문 관리 지역·역세권·생활권 안내 업데이트</description>\n"
    ...
    # robots.txt — 네이버(Yeti)·구글 등 전 봇 허용 + 사이트맵/RSS 안내
